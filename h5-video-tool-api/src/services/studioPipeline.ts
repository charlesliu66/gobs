...
function parseLlmJson<T>(raw: string, context: string): T {
  ...
  if (balanced && balanced !== text) chunks.unshift(balanced);
  ...
  for (const chunk of chunks) {
    ...
    try {
      return JSON.parse(jsonrepair(chunk)) as T;
    } catch (e) {
      lastErr = e;
    }
  }
...
export interface StoryCharacter {
  ...
  conflict: string;
  arc?: string;
  /** 戏剧功能；主角必须且仅一人标记为 protagonist，便于排序与资产页展示 */
  role?: 'protagonist' | 'supporting' | 'antagonist' | 'other';
}

export interface ScenePlanItem {
  id: string;
  /** 具体可拍摄的空间/地点（如：村庄祠堂、主角家中堂屋、雨夜公路），勿用节拍名充当场景名 */
  name: string;
  /** 该地点在叙事中的作用（可与节拍关联，但 name 本身必须是地点） */
  purpose: string;
  relatedBeatIds?: string[];
}

/** 剧本阶段即可锁定的重要道具，供服化道与分镜引用 */
...
const ROLE_ORDER: Record<NonNullable<StoryCharacter['role']>, number> = {
  protagonist: 0,
  supporting: 1,
  antagonist: 2,
  other: 3,
...
function normalizeStoryArcLayer(raw: StoryArcLayer): StoryArcLayer {
  ...
  for (const s of Array.isArray(raw.scenePlan) ? raw.scenePlan : []) {
    const id = ((s.id || '') as string).trim() || `loc-${scenePlan.length + 1}`;
    if (idSeen.has(id)) continue;
    idSeen.add(id);
    scenePlan.push({
      ...s,
      id,
      name: ((s.name || '') as string).trim() || id,
      purpose: ((s.purpose || '') as string).trim() || '—',
...
export async function generateStoryArc(input: {
  ...
  const parsed = parseLlmJson<StoryArcLayer>(raw, '故事结构 L1');
...
export async function generateProductionDesign(story: StoryArcLayer): Promise<ProductionDesignLayer> {
...
function scenePlanIdsMissingFromShots(
  shots: ProductionShot[],
  scenePlan: { id: string }[],
): string[] {
  const required = new Set(
    scenePlan.map((s) => String(s.id || '').trim()).filter(Boolean),
  );
  const used = new Set(
    shots.map((sh) => String(sh.sceneRef || '').trim()).filter(Boolean),
  );
...
export async function generateStoryboardTable(input: {
  ...
  const runOnce = async (userText: string) => {
    ...
    return parseLlmJson<{ shots?: ProductionShot[] }>(raw, '分镜表 L3');
  };

  let parsed = await runOnce(baseUser);
  if (!parsed.shots?.length) throw new Error('分镜表为空');

  const plan = input.story.scenePlan ?? [];
  let missing =
    plan.length > 0 && plan.length <= 14
  ...
  if (plan.length > 0 && plan.length <= 14 && missing.length > 0) {
    throw new Error(
      `分镜表仍未覆盖 L1 全部场景，以下 scenePlan.id 未出现在任何一镜的 sceneRef：${missing.join('、')}。请再次点击「生成分镜表」或精简 scenePlan 后重试。`,
    );
  }
...
export async function extractStyleReference(input: {
  ...
  const parsed = parseLlmJson<StyleReferenceAnalysis>(text, '风格反解析');
...
export async function extractCharacterVisuals(input: {
  ...
  return parseLlmJson<CharacterVisualProfile>(text, '角色视觉提取');
}

function joinStill(sp: StructuredPromptStill): string {
  return [
    `【主体】${sp.sp_subject}`,
...
function joinMotion(mp: StructuredPromptMotion): string {
  return [
    `【主体运动】${mp.mp_motion}`,
    `【摄影机】${mp.mp_camera}`,
    `【节奏】${mp.mp_tempo}`,
...
export function assemblePrompts(input: {
  ...
  targetPlatform?: 'seedance' | 'kling' | 'both';
}): AssemblePromptsResult {
  const prefix =
    input.characterVisualProfile?.consistencySnippet?.trim() ?
  ...
  const shots = input.shots.map((sh) => {
    ...
    const seedanceBlock =
      `${prefix}镜头${sh.shotIndex}｜${sh.durationSec}s｜${sh.shotScale}｜${sh.cameraMove}\n${still}\n---运动---\n${motion}\n@img1 角色参考 @img2 场景参考`;
    const klingPrompt = `${prefix}${still}\n${motion}`.replace(/\s+/g, ' ').trim();
    const nanoBananaStyle =
      input.characterVisualProfile ?
