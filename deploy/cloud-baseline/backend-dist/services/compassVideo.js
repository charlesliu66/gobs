...
function createClient(veoModel) {
    const apiKey = resolveCompassApiKeyForVeoModel(veoModel);
    return axios.create({
        baseURL: BASE_URL,
        timeout: 180000,
        headers: {
...
function toAspectRatio(aspectRatio) {
    const map = {
        '16:9': '16:9',
        '9:16': '9:16',
        '1:1': '1:1',
...
export async function createVideoTask(options) {
    ...
    const instances = [{ prompt: options.prompt.trim() }];
...
export async function waitForVideo(taskId, options = {}) {
    ...
    const result = { taskId, status: 'processing' };
    // 首次轮询前等待（与 Python SDK time.sleep(15) 一致）
    await new Promise((r) => setTimeout(r, 15000));
    while (Date.now() - start < maxWaitMs) {
        try {
            // operationName: 完整路径或 UUID（Compass 文档示例为 UUID，但完整路径也可能支持）
            const operationName = String(taskId).includes('/operations/') ? taskId : String(taskId).split('/operations/').pop() ?? taskId;
