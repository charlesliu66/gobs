# AGENTS.md — GOBS / QAS 项目智能体运行指南

> 兼容 Codex · Cursor · OpenClaw。首次读完后无需再追问项目背景。  
> **OpenAI Codex CLI 完整说明（不限篇幅、建议大任务显式引用）**：`docs/CODEX-CLI-PROJECT-GUIDE.md`
...
```
docs/DOCS-INDEX.md                    # 文档地图（先搞清楚每类文档各自负责什么）
...
**协作追踪**（每日复盘）：
- `docs/daily-reports/YYYY-MM-DD.md` — 每天完成后产出日报，追踪 Bug Fix 占比、同类问题重复、端到端验证等指标
...
#    - grep "NEXT_VERSION" PRODUCT.md 获取当前可用版本号
#    - 在"Changelog"顶部（NEXT_VERSION 标记下方）新增版本条目
#    - 将 NEXT_VERSION 标记递增（如 v0.54 → v0.55）
#    - 在"功能模块总览"章节更新对应功能描述
#    - 更新文件末尾的"最后更新"时间
...
| `DREAMINA_MAX_CONCURRENT` | 即梦同时生成并发数，默认 1（按账号能力调整） |
| `API_DATA_DIR` | 生产数据根目录（挂载云硬盘时设为挂载点） |
| `KLING_API_KEY` | 可灵视频生成（可选） |
| `SUNO_API_KEY` | Suno 音乐生成 API Key（不配置则用 Lyria） |
| `EDITOR_BEAT_ANALYSIS` | 设为 `1` 启用 AI 剪辑音乐先行节拍分析 |
