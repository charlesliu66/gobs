"""v0.58 部署：剪辑 Agent 多模型 Plan→Build + 前端快捷指令"""
import paramiko
import os
import sys
...
print('=== v0.58 部署: 剪辑 Agent 多模型 Plan→Build ===\n')
...
# 写入服务器环境变量（追加 Agent 模型配置）
print('\n=== 配置 Agent 模型环境变量 ===')
env_cmd = """
grep -q 'EDITOR_AGENT_PLAN_MODEL' /home/ubuntu/qas-h5/api/.env 2>/dev/null || cat >> /home/ubuntu/qas-h5/api/.env << 'ENVEOF'

# 剪辑 Agent 模型配置 (v0.58)
EDITOR_AGENT_PLAN_MODEL=DeepSeek-R1
EDITOR_AGENT_BUILD_MODEL=gpt-4.1
EDITOR_AGENT_FALLBACK_MODEL=gpt-4o
ENVEOF
...
ssh.close()
print(f'\n✅ v0.58 部署完成: api={api_count}, frontend={fe_count}')
