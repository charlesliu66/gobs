...
export function BossShowcase({ onBackToPicker }: { onBackToPicker?: () => void }) {
  ...
  const canStartGenerate = shotsValid;

  return (
    <div className="max-w-6xl space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="page-title">BOSS 展示</h2>
        {onBackToPicker && (
      ...
      <section>
        ...
        {tipsExpanded && (
          <div className="mt-2 p-3 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] text-xs text-[var(--color-text-muted)] space-y-2">
            ...
            <p><strong className="text-[var(--color-text)]">生成首尾帧：</strong>点击时按当前镜头描述文生图预览；素材图主要在生成视频时作参考，不再用素材图「垫图」覆盖分镜场景。</p>
          </div>
        )}
      </section>
