...
import { generateLyriaInstrumentalWavs, LYRIA_CLIP_DURATION_SEC, } from '../services/compassLyriaMusic.js';
...
router.post('/music/generate', async (req, res) => {
    ...
    try {
        const buffers = await generateLyriaInstrumentalWavs({
            ...
            sampleCount: typeof body.sampleCount === 'number' && Number.isFinite(body.sampleCount)
                ? body.sampleCount
                : 1,
        });
        const items = [];
