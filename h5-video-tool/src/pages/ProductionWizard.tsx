...
import {
  ensureExecutionSegments,
  getSegmentsForShot,
} from '../studio/executionSegments';
import { resolveSegmentStatus, resolveShotAggregateProviderStatus } from '../studio/executionSegmentStatus';
import {
...
import { ProductionProvider, type ProductionContextValue } from '../studio/ProductionContext';
...
import { ProductionWizardOverlayLayer, type ProductionWizardNamingModalState, type ProductionWizardOverlayLayerProps } from '../studio/steps/ProductionWizardOverlayLayer';
import { ProductionWizardStepContent, type ProductionWizardStepContentProps } from '../studio/steps/ProductionWizardStepContent';
import {
  PRODUCTION_WIZARD_ASPECT_OPTIONS,
  getProductionWizardFooterHint,
  getProductionWizardMaxReachableStep,
  getProductionWizardStepItems,
  getProductionWizardTemplateOptions,
  resolveProductionWizardBootstrapState,
  type ProductionWizardBatchAssetGenState,
  type ProductionWizardBatchTask,
  type ProductionWizardL2Tab,
} from '../studio/steps/productionWizardStepState';
...
export function ProductionWizard() {
  ...
  const formatAssetKind = useCallback((kind: ProductionWizardBatchTask['kind']) => {
  ...
  const {
    ...
    urlAssetId,
    urlProjectId,
  } = resolveProductionWizardBootstrapState(searchParams);
  ...
  const [l2Tab, setL2Tab] = useState<ProductionWizardL2Tab>('characters');
  ...
  const [projectNamingModal, setProjectNamingModal] = useState<ProductionWizardNamingModalState>({ open: false, name: '' });
  ...
  const localizedSteps = useMemo(
    () => getProductionWizardStepItems(t),
    [t],
  );
  const localizedTemplateOptions = useMemo(
    () => getProductionWizardTemplateOptions(t),
    [t],
  );
  ...
  const projectWithSegments = useMemo(() => ensureExecutionSegments(project), [project]);
  const segmentJobsMap = useMemo<Record<string, BatchJobDto[]>>(() => {
    const out: Record<string, BatchJobDto[]> = {};
    for (const job of projectJobs) {
      const key = job.segmentId?.trim();
      if (!key) continue;
  ...
  const shotActiveJobMap = useMemo<Record<string, BatchJobDto>>(() => {
    ...
    for (const shot of projectWithSegments.shots) {
      const relatedSegments = getSegmentsForShot(projectWithSegments, shot);
      const relatedJobs = [
        ...relatedSegments.flatMap((segment) => segmentJobsMap[segment.id] ?? []),
        ...projectJobs.filter((job) => job.primaryShotIndex === shot.shotIndex || job.shotIndex === shot.shotIndex),
      ].filter((job, idx, arr) => arr.findIndex((item) => item.id === job.id) === idx);
      const activeJobs = relatedJobs.filter((job) => !!activeRank[job.status]);
      if (!activeJobs.length) continue;
      byShot.set(shot.shotIndex, activeJobs);
    }
    ...
    return out;
  }, [projectJobs, projectWithSegments, segmentJobsMap]);

  const shotJobStatusMap = useMemo<Record<string, 'awaiting_submit' | 'queuing' | 'processing' | 'done' | 'failed' | 'cancelled'>>(() => {
    const out: Record<string, 'awaiting_submit' | 'queuing' | 'processing' | 'done' | 'failed' | 'cancelled'> = {};
    const byShot = new Map<number, BatchJobDto[]>();
    for (const shot of projectWithSegments.shots) {
      const relatedSegments = getSegmentsForShot(projectWithSegments, shot).map((segment) => ({
        ...segment,
        status: resolveSegmentStatus(segment, segmentJobsMap[segment.id] ?? []),
      }));
      const relatedJobs = [
        ...relatedSegments.flatMap((segment) => segmentJobsMap[segment.id] ?? []),
        ...projectJobs.filter((job) => job.primaryShotIndex === shot.shotIndex || job.shotIndex === shot.shotIndex),
      ].filter((job, idx, arr) => arr.findIndex((item) => item.id === job.id) === idx);
      byShot.set(shot.shotIndex, relatedJobs);
      const activeJob = shotActiveJobMap[String(shot.shotIndex)];
      if (activeJob) {
        if (activeJob.status === 'processing') out[String(shot.shotIndex)] = 'processing';
        else if (activeJob.status === 'awaiting_submit') out[String(shot.shotIndex)] = 'awaiting_submit';
        else out[String(shot.shotIndex)] = 'queuing';
        continue;
      }
      const aggregate = resolveShotAggregateProviderStatus(shot, relatedSegments);
      if (aggregate === 'done' || aggregate === 'failed' || aggregate === 'cancelled') {
        out[String(shot.shotIndex)] = aggregate;
      }
    }
    for (const [idx, arr] of byShot) {
      if (out[String(idx)]) continue;
    ...
    return out;
  }, [projectJobs, projectWithSegments, segmentJobsMap, shotActiveJobMap]);
  ...
  const [batchAssetGen, setBatchAssetGen] = useState<ProductionWizardBatchAssetGenState | null>(null);
  const batchCancelRef = useRef<boolean>(false);
  const failedTasksRef = useRef<ProductionWizardBatchTask[]>([]);
  ...
  const handleBatchGenerateMissingAssets = useCallback(async () => {
    ...
    const tasks: ProductionWizardBatchTask[] = [];
    ...
    const runTask = async (t: ProductionWizardBatchTask) => {
  ...
  useEffect(() => {
    ...
    const persistedProject = ensureExecutionSegments({
      ...project,
  ...
  const handleL3 = useCallback(async () => {
    ...
    try {
      ...
      setProject((p) => {
        ...
        return ensureExecutionSegments({ ...p, shots: merged, assembled: null });
      });
      setStep(3);
  ...
  const patchShot = useCallback((idx: number, patch: Partial<ProductionShot>) => {
    setProject((p) => {
      ...
      return ensureExecutionSegments({ ...p, shots, assembled: null });
    });
  }, []);
  ...
  const generateVideoForShotIdx = useCallback(async (shotIdx: number) => {
    const ensuredProject = ensureExecutionSegments(project);
    const s = ensuredProject.shots[shotIdx];
    if (!s) return;
    if (!serverProjectId) {
    ...
    try {
      ...
      const relatedSegments = getSegmentsForShot(ensuredProject, s);
      if (!relatedSegments.length) {
        setErr(t('productionWizard.shotPromptRequired'));
        return;
      }

      const multimodalPack = pref === 'dreamina-multimodal'
        ? await buildShotMultimodalRefPackAsync(
            s,
            project.characterAssets ?? [],
            project.sceneAssets ?? [],
            project.propAssets ?? [],
      ...
      let firstQueueInfo: { globalQueuePos: number; etaSec: number } | null = null;
      for (const segment of relatedSegments) {
        ...
        if (pref === 'dreamina-multimodal') {
          ...
          extraBody = { multimodalImages: multimodalPack?.multimodalImages ?? [] };
        } else {
        ...
        const submitReq = {
          ...
          primaryShotIndex: segment.primaryShotIndex,
          storyboardText,
          duration: Math.max(4, Math.min(15, Math.round(segment.durationSec || 6))),
          aspectRatio: ar,
          model,
        ...
        if (!firstQueueInfo) firstQueueInfo = { globalQueuePos, etaSec };
        ...
        if (enqueueState.isError) {
          ...
          setProject((p) => {
            const ensured = ensureExecutionSegments(p);
            const shots = [...ensured.shots];
            const cur = shots[shotIdx];
            if (!cur) return p;
            shots[shotIdx] = {
              ...cur,
      ...
      if (firstQueueInfo) {
        toast.success(
          formatText('productionWizard.shotQueued', {
            shotIndex: s.shotIndex,
            position: firstQueueInfo.globalQueuePos + 1,
            eta: formatEta(firstQueueInfo.etaSec),
          }),
        );
      }
    } catch (e) {
  ...
  const selectedShotJob = projectWithSegments.shots[selectedShotIdx]
    ? shotActiveJobMap[String(projectWithSegments.shots[selectedShotIdx].shotIndex)] ?? null
    : null;
  ...
  const shot = projectWithSegments.shots[selectedShotIdx];
  ...
  const footerHint = getProductionWizardFooterHint(step, t);
  const maxReachableStep = getProductionWizardMaxReachableStep(project);
  const productionContextValue = {
    ...
    setLightboxSrc,
    patchShot,
  } satisfies ProductionContextValue;
  const shellProps = {
    projectTitle: project.meta.title,
  ...
  const stepInputProps: ProductionWizardStepContentProps['stepInputProps'] = {
    ...
    aspectOptions: PRODUCTION_WIZARD_ASPECT_OPTIONS,
  ...
  const stepStoryArcProps: ProductionWizardStepContentProps['stepStoryArcProps'] = {
    styleRefSummary: project.meta.styleRefSummary,
    onStyleRefSummaryChange: (next) =>
      setProject((p) => ({ ...p, meta: { ...p.meta, styleRefSummary: next } })),
    aspectRatioText: ar,
  ...
  const stepDesignProps: ProductionWizardStepContentProps['stepDesignProps'] = {
  ...
  const stepStoryboardProps: ProductionWizardStepContentProps['stepStoryboardProps'] = {
    shot,
    shots: projectWithSegments.shots,
    chSheets,
    scSheets,
    ...
    shotJobQueueInfoMap,
    segmentJobsMap,
    executionSegments: projectWithSegments.executionSegments,
    ...
    busyL3,
    onSetShotVideoDreaminaModel: (value) =>
      setProject((p) => ({
        ...p,
        meta: {
          ...p.meta,
          shotVideoDreaminaModel: value ? value : undefined,
        },
      })),
    onSetDreaminaModelVersion: (value) =>
      setProject((p) => ({
        ...p,
        meta: { ...p.meta, dreaminaModelVersion: value ? value : undefined },
      })),
  ...
  const stepExportProps: ProductionWizardStepContentProps['stepExportProps'] = {
    shots: project.shots,
    scSheets,
  ...
  const stepContentProps: ProductionWizardStepContentProps = {
    step,
    story: story ?? null,
    hasProductionDesign: !!project.productionDesign,
    hasShots: project.shots.length > 0,
    busyL3,
    stepInputProps,
    stepStoryArcProps,
    stepDesignProps,
    stepStoryboardProps,
    stepExportProps,
    inputRequiredMessage: t('productionWizard.stepInputRequired'),
    storyRequiredMessage: t('productionWizard.stepStoryRequired'),
    designRequiredMessage: t('productionWizard.stepDesignRequired'),
  };
  const projectListModalProps: ProductionWizardOverlayLayerProps['projectListModalProps'] = {
  ...
  const continuousPlayProps: ProductionWizardOverlayLayerProps['continuousPlayProps'] = showContinuousPlay
    ? {
      shots: project.shots,
      resolveVideoSrc: resolveProductionShotPreviewVideoSrc,
    ...
    : null;
  const abCompareProps: ProductionWizardOverlayLayerProps['abCompareProps'] = showAbCompare
    ? {
      versions: shotVideoVersions,
      onClose: () => setShowAbCompare(false),
    }
    : null;
  const overlayProps: ProductionWizardOverlayLayerProps = {
    projectListModalProps,
    projectNamingModal,
    onProjectNamingChange: (nextName) =>
      setProjectNamingModal((current) => ({ ...current, name: nextName })),
    onCloseProjectNaming: closeProjectNamingModal,
    onConfirmProjectNaming: handleConfirmProjectNaming,
    projectNamingTitle: t('productionWizard.namingModalTitle'),
    projectNamingDescription: t('productionWizard.namingModalDesc'),
    projectNamingPlaceholder: t('productionWizard.namingPlaceholder'),
    cancelLabel: t('common.cancel'),
    confirmLabel: t('productionWizard.nameAndSave'),
    lightboxSrc,
  ...
  return (
    <>
      <ProductionProvider value={productionContextValue}>
        <ProductionWizardShell {...shellProps}>
          <ProductionWizardStepContent {...stepContentProps} />
        </ProductionWizardShell>
      </ProductionProvider>

      <ProductionWizardOverlayLayer {...overlayProps} />
    </>
  );
}
