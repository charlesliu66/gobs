...
async function resolveVideoInput(input, prefix) {
    if (isUrl(input)) {
    ...
    return new Promise((resolve, reject) => {
        ...
        proc.on('close', (code) => {
            if (code === 0)
                ...
                reject(new Error(`FFmpeg 执行失败: ${stderr.slice(-500)}`));
        });
...
/** 裁剪视频（trim） */
async function trimVideo(inputPath, outputPath, startSec, endSec) {
...
export async function runRemix(options) {
    ...
    try {
        ...
        if (subtitles && (typeof subtitles === 'string' ? subtitles.trim() : subtitles.length > 0)) {
            const srtContent = typeof subtitles === 'string'
                ? subtitles.trim()
                : cuesToSrt(subtitles);
 ...
 */
export async function runMergeRemix(options) {
    ...
    try {
        ...
        let finalPath;
        if (subtitles && (typeof subtitles === 'string' ? subtitles.trim() : subtitles.length > 0)) {
            const srtContent = typeof subtitles === 'string' ? subtitles.trim() : cuesToSrt(subtitles);
            const srtPath = path.join(tmpDir, 'subs.srt');
            await fs.writeFile(srtPath, srtContent, 'utf-8');
