...
/** 保证返回带前缀的 data URL */
function toPngDataUrl(rawBase64: string): string {
 ...
 */
function buildFramePrompts(sceneText: string, globalStyleLock: boolean): { first: string; last: string } {
  ...
  const lockZh = globalStyleLock
    ...
    : '';
  const en =
    'Generate exactly one image. The image must match the Chinese scene above (location, characters, action). ' +
    (globalStyleLock
 ...
 */
function buildContinuationPrompts(sceneText: string, phase: 'middle' | 'last', useGlobalBaseline: boolean): string {
  const scene = sanitizeFramePrompt(sceneText);
  const core = scene.length > 0 ? scene : '场景描述';
  ...
  const en =
    'Generate exactly one image for this shot. Match the Chinese scene. ' +
    stylePhrase +
 ...
 */
storyboardRouter.post('/portrait', async (req: Request, res: Response) => {
  const { prompt, aspectRatio = '9:16', referenceImage, compassApiKey, globalStyleReferenceFrame } =
    req.body as {
      prompt?: string;
      aspectRatio?: string;
  ...
  const portraitOutputLock =
    '【输出规格·角色定妆】全身照：从头到脚完整入镜，正面平视镜头，人物全身居中；背景纯白色 #FFFFFF，无真实场景/道具/渐变；禁止非白底。';
