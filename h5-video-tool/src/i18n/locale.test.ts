...
test('message lookup resolves English keys and falls back to Chinese', () => {
  ...
  assert.equal(getMessage('en', 'generate.pageTitle'), 'Generate Video');
  assert.equal(getMessage('en', 'generate.inputTitle'), 'Creative Description');
  assert.equal(getMessage('en', 'generate.referenceAssetsTitle'), 'Reference Assets');
  assert.equal(getMessage('en', 'generate.moreAssetSources'), 'More asset sources');
  assert.equal(getMessage('en', 'generate.matchAssets'), 'Match Assets');
  assert.equal(getMessage('en', 'generate.browseTemplateIdeas'), 'Templates');
  assert.equal(getMessage('en', 'generate.tiktokReferenceVideo'), 'Motion Reference Video');
  assert.equal(getMessage('en', 'generate.configureDriveHint'), 'Set a Drive folder in Materials to use legacy Drive matching.');
  assert.equal(getMessage('zh-CN', 'generate.pageTitle'), '生成视频');
  assert.equal(getMessage('zh-CN', 'generate.referenceAssetsTitle'), '参考素材');
