...
import { loadAssetIndex, saveAssetIndex, buildAssetIndex, autoTagImage, } from '../services/assetLibrary.js';
 ...
 */
assetsRouter.post('/auto-tag', async (req, res) => {
 ...
 * 返回素材图片的 base64 数据
 */
assetsRouter.get('/:id/image', async (req, res) => {
    const { id } = req.params;
    try {
        const index = await loadAssetIndex();
...
assetsRouter.put('/:id', async (req, res) => {
    ...
    try {
        ...
        const allowed = ['name', 'type', 'tags', 'gameVersion', 'description'];
        for (const key of allowed) {
            if (key in updates) {
                index.assets[idx][key] = updates[key];
            }
        }
        await saveAssetIndex(index);
