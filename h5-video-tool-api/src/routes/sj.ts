...
import {
  ...
  taskDetail,
  taskCancel,
  scheduleTiktokComment,
} from '../services/sjGeelark.js';
...
function toScheduleAt(p: { scheduleAt?: number; scheduleDate?: string; scheduleTime?: string; timezone?: string }): number | null {
...
sjRouter.get('/geelark-status', async (_req: Request, res: Response) => {
...
/** GET /api/sj/phones */
sjRouter.get('/phones', async (_req: Request, res: Response) => {
  try {
...
sjRouter.post('/phones/start', async (req: Request, res: Response) => {
...
sjRouter.post('/phones/stop', async (req: Request, res: Response) => {
  const { ids } = req.body as { ids?: string[] };
  if (!Array.isArray(ids) || ids.length === 0) {
    return res.status(400).json({ error: '请提供 ids 数组' });
  }
...
sjRouter.post('/phones/delete', async (req: Request, res: Response) => {
  const { ids } = req.body as { ids?: string[] };
  if (!Array.isArray(ids) || ids.length === 0) {
    return res.status(400).json({ error: '请提供 ids 数组' });
  }
...
sjRouter.patch('/phones/update', async (req: Request, res: Response) => {
  const { id, name, remark, proxyId } = req.body as { id?: string; name?: string; remark?: string; proxyId?: string };
  if (!id) {
...
/** GET /api/sj/proxies */
sjRouter.get('/proxies', async (req: Request, res: Response) => {
...
sjRouter.post('/proxies', async (req: Request, res: Response) => {
  const body = req.body as { list?: { scheme: string; server: string; port: number; username?: string; password?: string }[]; id?: string; scheme?: string; server?: string; port?: number; username?: string; password?: string };
...
sjRouter.delete('/proxies', async (req: Request, res: Response) => {
...
sjRouter.post('/schedule', async (req: Request, res: Response) => {
  const { tasks } = req.body as { tasks?: { phoneId: string; videoLink: string; comment: string; scheduleAt?: number; scheduleDate?: string; scheduleTime?: string; timezone?: string; useAsia?: boolean }[] };
...
sjRouter.post('/generate-comment', async (req: Request, res: Response) => {
  ...
  if (apiKey) {
    try {
      ...
      if (fetchRes.ok) {
        const data = (await fetchRes.json()) as { choices?: { message?: { content?: string } }[] };
        const text = data.choices?.[0]?.message?.content?.trim() || '';
        const lines = text.split('\n').map((s: string) => s.replace(/^\d+[.)]\s*/, '').trim()).filter(Boolean).slice(0, 5);
        if (lines.length > 0) return res.json({ suggestions: lines });
      }
    } catch {}
  }
...
sjRouter.get('/tasks/history', async (req: Request, res: Response) => {
  const size = Math.min(parseInt(req.query.size as string, 10) || 100, 100);
  try {
...
sjRouter.post('/tasks/query', async (req: Request, res: Response) => {
  const { ids } = req.body as { ids?: string[] };
  if (!Array.isArray(ids) || ids.length === 0) {
...
sjRouter.post('/tasks/detail', async (req: Request, res: Response) => {
  const { id, searchAfter } = req.body as { id?: string; searchAfter?: number[] };
  if (!id) {
    return res.status(400).json({ error: '请提供 id' });
  }
  try {
    const data = await taskDetail(id, searchAfter as number[] | undefined);
    res.json(data);
  } catch (e) {
...
sjRouter.post('/tasks/cancel', async (req: Request, res: Response) => {
  const { ids } = req.body as { ids?: string[] };
  if (!Array.isArray(ids) || ids.length === 0) {
    return res.status(400).json({ error: '请提供 ids 数组' });
  }
