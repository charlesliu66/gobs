/**
 * 解析 promptPolish 输出的分镜格式，提取镜头列表
 * 格式: [00:00-00:03] 镜头1：悬念（刀刃特写，缓慢推进）。雨滴打在刀刃上...
 */
export interface ParsedShot {
  index: number;
  timeRange: string;      // "00:00-00:03"
  startSec: number;       // 0
  endSec: number;         // 3
  durationSeconds: number;
  name: string;           // "悬念"
  prompt: string;         // 完整描述，用于生成分镜图
}

/** 匹配 [00:00-00:03] 镜头1：名称（景别，运镜）。描述... */
const SHOT_REGEX = /\[(\d{2}):(\d{2})-(\d{2}):(\d{2})\]\s*镜头\d+[：:]\s*(.+?)[。.]([\s\S]*?)(?=\[\d{2}:\d{2}-\d{2}:\d{2}\]|$)/g;

export function parseStoryboard(storyboardText: string): ParsedShot[] {
 ...
 */
export function parseOrSingleShot(storyboardText: string, fallbackPrompt?: string): ParsedShot[] {
