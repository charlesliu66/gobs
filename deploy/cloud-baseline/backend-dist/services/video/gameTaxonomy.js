...
let cached = null;
function resolveTaxonomyFilePath() {
...
export function loadGameTaxonomy() {
    ...
    try {
        ...
        const data = {
            ...
            roles: Array.isArray(raw.roles) ? raw.roles.filter((x) => typeof x === 'string') : [],
            scenes: Array.isArray(raw.scenes) ? raw.scenes.filter((x) => typeof x === 'string') : [],
            activities: Array.isArray(raw.activities)
                ? raw.activities.filter((x) => typeof x === 'string')
                : [],
            referenceImagesRoot: typeof raw.referenceImagesRoot === 'string' ? raw.referenceImagesRoot.trim() : undefined,
        };
 ...
 */
export function buildReferenceAnchorParts(taxonomy) {
    ...
    const parts = [];
    let total = 0;
    const pushDir = (segment, label, kindCn) => {
        ...
        for (const f of files) {
            if (n >= maxPerLabel || total >= maxTotal)
                break;
...
export function formatTaxonomyForPrompt(t) {
    ...
    const lines = [];
    if (t.gameName)
        lines.push(`游戏/项目：${t.gameName}`);
    if (t.description)
        lines.push(t.description);
    if (t.roles.length)
        lines.push(`可选职业/角色标签（从中选 0~2 个最符合的，无法判断填 []）：${t.roles.join('、')}`);
    if (t.scenes.length)
        lines.push(`可选场景标签（选 0~1 个）：${t.scenes.join('、')}`);
    if (t.activities.length)
        lines.push(`可选行为标签（搜打撤类：选 1 个最符合当前画面的）：${t.activities.join('、')}`);
    if (t.referenceImagesRoot) {
        lines.push(`已启用参考图目录（仅首屏批次附带）：${t.referenceImagesRoot} 下 roles/<职业>/、scenes/<场景>/`);
    }
    return lines.join('\n');
}
