...
第一次请求searchAfter不用传，如果响应的logContinue为true，代表日志需要分页返回，此时可以将响应的searchAfter作为参数传递。

## 请求URL

- `https://openapi.geelark.cn/open/v1/task/detail`
...
| 参数名       | 必选   |     类型   |    说明    | 示例|
| ----------- | -------| -----------|----------- |----------- |
| id       | 是     |   string  | 任务id | 1234567898 |
| searchAfter | 否 | object | 日志分页参数 | 参考请求示例 |
...
| taskType  | integer    | 任务类型<br>1 TikTok发布视频<br>2 TikTok AI养号<br>3 TikTok发布图集<br>4 TikTok登录账号<br>6 TikTok编辑资料<br>42 自定义任务(包括Facebook, YouTube等平台) |
| serialName  | string    | 云手机名 |
| envId  | string    | 云手机id |
| scheduleAt  | integer    | 计划时间，秒级时间戳 |
| status | integer | 任务状态<br>1 等待执行<br>2 执行中<br>3 任务完成<br>4 任务失败<br>7 任务取消 |
| failCode | integer | 失败代码，参考任务失败代码及原因 |
| failDesc | string | 失败原因，参考任务失败代码及原因 |
| cost | integer | 任务完成或失败时，执行所用的时间，单位：秒 |
| resultImages | array[string] | 任务完成或失败时，截图信息 |
| logs | array[string] | 任务日志，时间为0时区，任务完成或失败时返回 |
| searchAfter | object | 日志分页参数 |
| logContinue | bool | 是否还有日志 |
...
    "data": {
		...
		"cost": 60,
		"resultImages": [
            "https://material.geelark.cn/geeUserUix/569544577254450679_1929760564478226432_20250604104103.jpg"
        ],
        "logs": [
