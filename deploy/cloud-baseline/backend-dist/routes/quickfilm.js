...
import { addJob } from '../services/batchJobsQueue.js';
 ...
 */
quickfilmRouter.post('/start', async (req, res) => {
    const { story, protagonist, protagonistDesc, style, styleImageBase64, assetFiles, } = req.body;
 ...
 */
quickfilmRouter.get('/:jobId/status', (req, res) => {
 ...
 */
quickfilmRouter.post('/:jobId/confirm', async (req, res) => {
    const { jobId } = req.params;
    ...
    fs.writeFileSync(path.join(getApiDataDir(), 'quickfilm', `${jobId}.json`), JSON.stringify(job, null, 2), 'utf-8');
...
function getDraftsDir(username) {
    if (!isSafeId(username))
...
draftsRouter.get('/', async (req, res) => {
    ...
    try {
        ...
        const drafts = files.map((f) => {
            ...
            return { id: raw.id, name: raw.name, updatedAt: raw.updatedAt, createdAt: raw.createdAt };
        });
