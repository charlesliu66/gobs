...
import { generateCaptionForPost, type CaptionByPlatformResult } from '../api/promptPolish';

export function TabDistribute() {
  const { videoUrl, videoPath, prompt } = useCreateFlow();
  ...
  const handleGenerateCaption = async () => {
    ...
    if (!raw && !hasExisting) {
      setCaptionGenError('请先在 Studio 输入 prompt，或在文案/标签中填写待优化内容');
      return;
    }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <h1 className="text-xl font-semibold text-[var(--color-text)] mb-4">视频分发</h1>

      {/* TT 账号列表 */}
      <section className="mb-6">
        <h2 className="text-sm font-medium text-[var(--color-text)] mb-3">选择目标账号</h2>
        {loading ? (
      ...
      {/* 视频与文案 */}
      <section className="mb-6">
        <h2 className="text-sm font-medium text-[var(--color-text)] mb-3">视频与文案</h2>
        {videoUrl ? (
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button
                  ...
                  disabled={
                    captionGenLoading ||
                    (!(prompt?.trim()) && !(caption?.trim()) && !(hashtags?.trim()))
                  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 flex items-center justify-between p-4 border-b border-[var(--color-border)] bg-[var(--color-surface-elevated)]">
          <h2 className="text-lg font-semibold text-[var(--color-text)]">运行报告</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-4 space-y-6">
          {loading ? (
            ...
            <>
              {/* 运行信息 */}
              <section>
                <h3 className="text-sm font-medium text-[var(--color-text)] mb-3">运行信息</h3>
              ...
              {report.resultImages && report.resultImages.length > 0 && (
                <section>
                  <h3 className="text-sm font-medium text-[var(--color-text)] mb-3">任务结束截图</h3>
                  <div className="space-y-2">
