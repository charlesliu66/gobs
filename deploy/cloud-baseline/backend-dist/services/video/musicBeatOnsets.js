...
function onsetStrengthSeries(samples, sampleRate) {
 ...
 */
export function pickBeatCutPointsFromSamples(samples, sampleRate, opts) {
...
export function beatPointsToMicroWindows(points, halfWidthSec) {
...
export async function analyzeBeatOnsetCuts(videoPath, maxAnalyzeSec) {
    const maxA = Math.min(900, maxAnalyzeSec ?? Number(process.env.EDITOR_ANALYSIS_MAX_SEC || 900));
