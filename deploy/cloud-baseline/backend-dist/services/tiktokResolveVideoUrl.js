...
export function isResolvableSocialVideoPageUrl(urlStr) {
    try {
        const h = new URL(urlStr).hostname.toLowerCase();
        if (h === 'tiktok.com' || h.endsWith('.tiktok.com'))
            return true;
        if (h === 'vm.tiktok.com' || h === 'vt.tiktok.com' || h === 'm.tiktok.com')
            return true;
        if (h.includes('douyin.com') || h.includes('iesdouyin.com'))
            return true;
        return false;
    }
    catch {
...
function spawnYtDlp(args, timeoutMs) {
    ...
    return new Promise((resolve, reject) => {
        ...
        proc.on('error', (err) => {
            clearTimeout(timer);
            if (err.code === 'ENOENT') {
                reject(new Error('未找到 yt-dlp。请安装：`py -m pip install -U yt-dlp`，或设置 YT_DLP_PATH 指向 yt-dlp.exe。'));
                return;
            }
            reject(err);
        });
 ...
 */
export async function resolveSocialPageToDirectVideoUrl(pageUrl) {
    const args = [
        '-g',
        '--no-playlist',
        '--no-warnings',
    ...
    if (code !== 0) {
        throw new Error(`无法解析直链（yt-dlp 退出码 ${code}）。${stderr.trim().slice(0, 400)} 请升级：py -m pip install -U yt-dlp`);
    }
    const direct = stdout
        .split(/\r?\n/)
        .map((l) => l.trim())
    ...
    if (!direct)
        throw new Error('yt-dlp 未返回有效 http(s) 地址');
    return direct;
}
/**
 * 用 yt-dlp 将分享页视频下载到 kling-ref-cache，返回本地路径与 cacheId。
 */
export async function downloadSocialPageVideoToKlingCache(pageUrl) {
    ...
    if (code !== 0) {
        throw new Error(`下载参考视频失败（yt-dlp 退出码 ${code}）。${stderr.trim().slice(0, 500)} 可尝试升级 yt-dlp 或检查 YT_DLP_PROXY。`);
    }
...
/** 约 30 分钟后删除临时文件 */
export function scheduleRefCacheCleanup(absPath) {
 ...
 */
export async function prepareSocialVideoUrlForKling(pageUrl) {
    const base = getPublicApiBaseUrlForKlingRef();
    if (!base) {
        throw new Error('参考视频仅支持 URL。使用 TikTok/抖音链接时请在 .env 配置 API_PUBLIC_BASE_URL（或 H5_PUBLIC_API_BASE_URL / KLING_REF_VIDEO_PUBLIC_BASE），' +
...
export async function findRefCacheFile(cacheId) {
