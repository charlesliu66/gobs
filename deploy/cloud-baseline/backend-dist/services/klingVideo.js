 ...
 */
function quoteJsonBigIntTaskIds(jsonStr) {
    return jsonStr
...
function klingAxiosConnectionConfig() {
 ...
 */
function normalizeImageB64(s) {
    return s.replace(/^data:image\/\w+;base64,/, '');
}
/** 合并单张与多张字段 */
function resolveRefImages(options) {
...
function resolveRefVideos(options) {
    const list = options.videoList?.filter((v) => typeof v.videoUrl === 'string' && v.videoUrl.trim().length > 0);
    return list ?? [];
}
/** 参考视频仅允许 URL，禁止把整段 MP4 当 base64 传入 */
function assertRefVideoUrl(raw) {
    const video_url = raw.trim();
    if (/^https?:\/\//i.test(video_url))
        return video_url;
    throw new Error('参考视频 video_list 仅支持 **http(s) URL**（可灵侧需能 GET 拉取），不支持 base64 或 data: 内嵌。请使用 API_PUBLIC_BASE_URL + /api/video/kling/ref-cache/:id、或 OSS/CDN 公网链接。');
}
/** Omni video_list 项（官方：video_url 为可拉取 URL） */
function toOmniVideoListPayload(items) {
    return items.map((v) => ({
        video_url: assertRefVideoUrl(v.videoUrl),
        refer_type: (v.referType === 'base' ? 'base' : 'feature'),
        keep_original_sound: v.keepOriginalSound === 'yes' ? 'yes' : 'no',
    }));
}
/** clipai.ingarena：video_url 与官方一致，仅 https 或 http URL */
function toIngarenaVideoListPayload(items) {
    return items.map((v) => ({
        video_url: assertRefVideoUrl(v.videoUrl),
        refer_type: (v.referType === 'base' ? 'base' : 'feature'),
        keep_original_sound: v.keepOriginalSound === 'yes' ? 'yes' : 'no',
...
function toOmniImageListPayload(refs) {
 ...
 */
function toIngarenaImageListPayload(refs) {
    return refs.map((item) => {
        const b64 = normalizeImageB64(item.imageBase64);
        const cell = {
            image_url: b64,
...
function mapIngarenaOmniModelName(model) {
    const m = model.trim().toLowerCase();
    if (m === 'kling-video-o1')
        return 'kling-video-o1';
    if (m === 'kling-v3-omni' || m === 'kling-v3')
        return 'kling-v3-omni';
    if (m.startsWith('kling'))
        return 'kling-v3-omni';
    return 'kling-v3-omni';
}
function extractIngarenaVideoRows(data) {
...
function ingarenaTaskFailed(status) {
    if (status === 3)
        return true;
...
function ingarenaTaskSucceed(status) {
    if (status === 2)
        return true;
    if (status === '2')
        return true;
    if (typeof status === 'string') {
        const s = status.toLowerCase();
        if (s === 'succeed' || s === 'success' || s === 'completed' || s === 'done')
            return true;
        const n = parseInt(s, 10);
        if (!Number.isNaN(n) && n === 2)
            return true;
    }
    if (typeof status === 'number' && Number.isFinite(status) && Math.trunc(status) === 2)
        return true;
    return false;
}
/** 部分网关状态码未文档化，但已有可播放地址且非失败态，视为成功 */
function ingarenaRowLooksReady(row) {
...
async function findIngarenaRowByTaskId(http, tid) {
...
function mapAspectRatio(ar) {
    ...
    if (a === '9:16' || a === '16:9' || a === '1:1')
        return a;
    return '16:9';
}
/** Omni 常用档位：5 / 10 / 15（秒）；超出则夹到最近档 */
function mapDurationSec(sec) {
...
/** 官方 mode：std | pro */
function mapOfficialMode(raw) {
    const x = raw.trim().toLowerCase();
    if (x === 'pro' || x === 'professional')
        return 'pro';
    return 'std';
}
/** 网关 mode：standard | professional 等 */
function mapGatewayMode(raw) {
    const x = raw.trim().toLowerCase();
    if (x === 'professional' || x === 'pro')
        return 'professional';
    if (x === 'std')
        return 'standard';
...
function extractTaskId(data) {
    ...
    if ((code === 0 || code === '0') && o.data && typeof o.data === 'object') {
        ...
        if (Array.isArray(tasks) && tasks[0] && typeof tasks[0] === 'object') {
            const tid = readTaskIdField(tasks[0].task_id);
            if (tid)
                return tid;
        }
    }
...
function extractStatusAndUrl(data) {
    ...
    const tryVideoFromOfficial = (obj) => {
        ...
        if (tr && typeof tr === 'object') {
            ...
            if (typeof t.video_url === 'string')
                return t.video_url;
        }
        return null;
    };
    const tryRead = (obj) => {
        const st = (obj.task_status ?? obj.status ?? obj.state ?? obj.task_status_msg ?? '');
        let url = tryVideoFromOfficial(obj);
        if (!url) {
            if (typeof obj.video_url === 'string')
                url = obj.video_url;
            else if (typeof obj.videoUrl === 'string')
                url = obj.videoUrl;
            else {
                const out = obj.output ?? obj.result ?? obj.task_result;
                if (out && typeof out === 'object') {
                    const ou = out;
                    if (typeof ou.video_url === 'string')
                        url = ou.video_url;
                    else if (typeof ou.videoUrl === 'string')
                        url = ou.videoUrl;
                    else if (typeof ou.url === 'string')
                        url = ou.url;
                }
            }
        }
...
function throwIfOfficialBusinessError(data, label) {
    if (!data || typeof data !== 'object')
        return;
    const o = data;
    const c = o.code;
...
function pollPathForKind(baseURL, kind, taskId) {
    if (isOfficialKlingBase(baseURL)) {
        if (kind === 'omni')
            return `/v1/videos/omni-video/${encodeURIComponent(taskId)}`;
...
function enrichIngarenaOmniError(message) {
...
async function klingSubmitTask(options) {
    ...
    if (ingarena) {
        ...
        try {
            ...
            if (e instanceof Error)
                throw new Error(enrichIngarenaOmniError(e.message));
            throw e;
        }
        taskId = extractTaskId(data);
        if (!taskId) {
            throw new Error(`可灵 omni-video 未解析到 task_id，请对照网关文档检查响应字段。原始响应: ${JSON.stringify(data).slice(0, 4000)}`);
        }
    }
    else if (official) {
        const callback_url = process.env.KLING_CALLBACK_URL ?? '';
        ...
        if (refImages.length || refVideos.length) {
            ...
            if (refImages.length)
                body.image_list = toOmniImageListPayload(refImages);
            if (refVideos.length)
                body.video_list = toOmniVideoListPayload(refVideos);
...
export async function createKlingVideoTaskOnly(options) {
    const { taskId } = await klingSubmitTask(options);
    return taskId;
}
export async function generateKlingVideo(options) {
    const { baseURL, axios: http } = getClient();
    ...
    if (ingarena) {
        ...
        throw new Error('可灵任务超时未完成，请稍后重试或缩短时长。可在 .env 增大 KLING_POLL_MAX（默认 600 次）或 KLING_POLL_MS，或到平台后台查看该任务是否仍在排队/失败。');
    }
...
function normalizeTaskIdParam(raw) {
    const s = raw.trim();
    if (s.toLowerCase().startsWith('kling-'))
        return s.slice(6);
    return s;
}
function mapIngarenaRowToPublic(row) {
    ...
    const modeLabel = typeof modeRaw === 'string'
        ? modeRaw
        : modeRaw != null
            ? String(modeRaw)
            : undefined;
    ...
    if (typeof snd === 'boolean')
        soundLabel = snd ? '有声' : '无声';
    else if (typeof snd === 'string' || typeof snd === 'number')
        soundLabel = String(snd);
    const createdAt = (typeof row.create_time === 'string' && row.create_time) ||
        (typeof row.created_at === 'string' && row.created_at) ||
        (typeof row.createTime === 'string' && row.createTime) ||
        undefined;
    const prompt = typeof row.prompt === 'string' && row.prompt.trim() ? row.prompt.trim() : undefined;
    return {
        taskId,
...
export async function fetchIngarenaVideoListPage(options) {
    ...
    return rows.map((r) => mapIngarenaRowToPublic(r));
}
/**
 * 查询 ingarena video-list 中某 task 的状态（供 H5 每 5 分钟轮询）。
 * 未出现在列表中视为仍排队：phase = pending。
 */
export async function queryIngarenaKlingTask(rawTaskId) {
 ...
 */
export async function probeIngarenaKlingVideoList() {
    const baseURL = (process.env.KLING_API_BASE_URL || DEFAULT_BASE).replace(/\/+$/, '');
    ...
    const token = process.env.KLING_API_KEY?.trim();
    if (!token)
        throw new Error('缺少 KLING_API_KEY');
    ...
    const preview = typeof r.data === 'string' ? r.data.slice(0, 600) : JSON.stringify(r.data).slice(0, 800);
