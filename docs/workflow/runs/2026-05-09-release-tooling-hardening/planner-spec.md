...
- Deterministic SSH/SFTP behavior in `deploy_api.py` and `deploy_frontend.py`, including finite channel timeouts, clearer progress, resource cleanup in `finally`, and non-zero remote command failures surfacing as local errors.
...
- Deploy SSH/SFTP helpers:
  - Responsibilities: Connect with bounded timeouts, upload local `dist/` trees, run remote PM2 commands, and close SSH/SFTP resources predictably.
...
- SSH/SFTP reliability: add explicit connect/channel timeouts, keepalive, progress logging, `finally` cleanup, and remote command stderr/exit-code handling. The scripts should fail loudly when a remote command fails, rather than waiting indefinitely.
...
|---|---|---|---|---|
| Timeout too aggressive | Slow SFTP upload over weak network | False deployment failure | Use conservative 120s channel timeouts and keep progress visible | Builder |
...
| AC-02 | deploy_api.py and deploy_frontend.py close SSH/SFTP resources deterministically and do not hang after successful uploads or PM2 restarts. | Unit tests for helper configuration plus real staging deployment observation. | Upload scripts print progress, enforce finite SSH/SFTP waits, close resources in `finally`, and finish staging deployment without manual kill. |
