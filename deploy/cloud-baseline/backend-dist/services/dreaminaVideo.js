...
export function isDreaminaModel(model) {
    const m = model?.trim().toLowerCase() ?? '';
    return m === DREAMINA_TEXT2VIDEO || m === DREAMINA_IMAGE2VIDEO || m === DREAMINA_MULTIMODAL;
}
export function isDreaminaMultimodalModel(model) {
    return model?.trim().toLowerCase() === DREAMINA_MULTIMODAL;
}
export function getDreaminaModelIds() {
    return [DREAMINA_MULTIMODAL, DREAMINA_TEXT2VIDEO, DREAMINA_IMAGE2VIDEO];
}
export function isDreaminaEnabled() {
    if (process.env.DREAMINA_ENABLED === '0' || process.env.DREAMINA_ENABLED === 'false')
        return false;
    if (process.env.DREAMINA_ENABLED === '1' || process.env.DREAMINA_ENABLED === 'true')
        return true;
    try {
...
export function getDreaminaScriptsDir() {
...
function defaultWindowsPythonExe() {
 ...
 */
function getDreaminaExecutableForWrapper() {
...
const DREAMINA_NOT_FOUND_HINT = '未找到 dreamina 命令。请安装官方 CLI（见 dreamina-cli-skill README），或在 h5-video-tool-api/.env 设置 DREAMINA_BIN=可执行文件绝对路径（Windows 可指向 Git Bash 安装后的 ~/.local/bin/dreamina），然后重启 API。';
function dreaminaFailureMessage(raw) {
 ...
 */
function normalizeDreaminaSeedanceVersion(mv) {
    const v = mv?.trim();
    if (!v)
        return undefined;
    if (v === 'seedance2.0_vip')
        return 'seedance2.0';
    return v;
}
/** text2video：ratio 映射为 CLI 支持的枚举 */
function aspectToDreaminaRatio(aspect) {
 ...
 */
function toDreaminaCliPath(absPath) {
 ...
 */
function getDreaminaStagingDir() {
    const env = process.env.DREAMINA_MEDIA_STAGING_DIR?.trim();
    if (env)
        return path.resolve(env);
    return getUploadsPath('dreamina-mm');
}
function parseWrapperStdout(stdout) {
    const t = stdout.trim();
    const start = Math.min(t.indexOf('{') >= 0 ? t.indexOf('{') : Infinity, t.indexOf('[') >= 0 ? t.indexOf('[') : Infinity);
...
async function runWrapper(script, args) {
    const scriptsDir = getDreaminaScriptsDir();
    const scriptPath = path.join(scriptsDir, script);
    if (!fsSync.existsSync(scriptPath)) {
        throw new Error(`未找到即梦脚本：${scriptPath}。请设置 DREAMINA_SCRIPTS_DIR，或将 dreamina-cli-skill 置于 QAS/.cursor/skills/。`);
    }
    ...
    try {
        ...
        throw new Error(`即梦输出无法解析为 JSON。stdout=${stdout.slice(0, 500)} stderr=${stderr.slice(0, 300)}`);
    }
}
function findMp4PathInObject(obj, depth = 0) {
    if (depth > 8 || obj == null)
        return null;
    if (typeof obj === 'string') {
        const s = obj.trim();
        if (/\.mp4$/i.test(s) && (s.includes('/') || s.includes('\\')))
            return s;
        return null;
    }
...
async function readVideoAsDataUrlFromDreaminaData(data) {
    if (!data)
        return null;
...
async function queryResultDownload(submitId) {
 ...
 */
export async function pollDreaminaTask(submitId) {
...
export async function submitDreaminaMultimodalVideo(options) {
    ...
    if (!prompt)
        throw new Error('请提供 prompt（可用 @图片1 @视频1 等引用素材）');
    const images = options.images ?? [];
    const videos = options.videos ?? [];
    const audios = options.audios ?? [];
    if (images.length > 9)
        throw new Error('参考图最多 9 张（即梦 CLI 限制）');
    if (videos.length > 3)
    ...
    const mv = normalizeDreaminaSeedanceVersion(options.modelVersion) ||
    ...
    try {
        ...
        const write = async (item, kind) => {
            const b64 = stripDataUrlBase64(item.base64);
            if (!b64)
                throw new Error('素材 base64 为空');
            const ext = extForMime(item.mimeType, kind);
            const p = path.join(staging, `dreamina_mm_${kind}_${Date.now()}_${randomBytes(4).toString('hex')}.${ext}`);
        ...
        for (const im of images)
            imagePaths.push(await write(im, 'image'));
        const videoPaths = [];
        for (const v of videos)
            videoPaths.push(await write(v, 'video'));
        const audioPaths = [];
        for (const a of audios)
            audioPaths.push(await write(a, 'audio'));
        const args = [
            '--prompt',
            prompt,
            '--duration',
        ...
        if (!submitId)
            throw new Error('即梦未返回 submit_id');
 ...
 */
export async function submitDreaminaVideo(options) {
    ...
    if (m === DREAMINA_IMAGE2VIDEO) {
        ...
        const ext = options.imageMimeType?.includes('png') ? 'png' : options.imageMimeType?.includes('webp') ? 'webp' : 'png';
        const staging = getDreaminaStagingDir();
        await fs.mkdir(staging, { recursive: true });
 ...
 */
export async function generateDreaminaVideo(options) {
    const m = options.model.trim().toLowerCase();
    if (m === DREAMINA_MULTIMODAL) {
    ...
    if (m === DREAMINA_IMAGE2VIDEO) {
        ...
        try {
            ...
            let videoUrl = await readVideoAsDataUrlFromDreaminaData(data);
            if (!videoUrl)
                videoUrl = await queryResultDownload(submitId);
...
function extForMime(mime, kind = 'image') {
...
export async function generateDreaminaMultimodalVideo(options) {
    ...
    try {
        ...
        for (const a of audios)
            audioPaths.push(await write(a, 'audio'));
        const args = ['--prompt', prompt, '--duration', String(duration), '--ratio', ratio, '--video-resolution', '720p', '--model-version', mv, '--poll', poll];
        for (const p of imagePaths) {
            args.push('--image', p);
        }
        for (const p of videoPaths) {
