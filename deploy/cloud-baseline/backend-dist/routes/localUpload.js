...
const MAX_MB = Math.min(4096, Math.max(64, parseInt(process.env.EDITOR_UPLOAD_MAX_MB || '500', 10) || 500));
// multer 配置
...
(async () => {
    try {
        ...
        for (const f of files) {
            ...
            const kind = /\.(jpe?g|png|webp|gif)$/.test(ext) ? 'image'
...
// ── 上传 ─────────────────────────────────────────────────────────────────────
localUploadRouter.post('/local', (req, res) => {
...
// ── 读取文件 ─────────────────────────────────────────────────────────────────
localUploadRouter.get('/file/:id', async (req, res) => {
    const { id } = req.params;
...
// ── 列出文件 ─────────────────────────────────────────────────────────────────
localUploadRouter.get('/list', (_req, res) => {
...
// ── 删除文件 ─────────────────────────────────────────────────────────────────
localUploadRouter.delete('/:id', async (req, res) => {
    const { id } = req.params;
    const record = uploadRegistry.get(id);
    if (!record) {
        res.status(404).json({ error: '文件不存在' });
