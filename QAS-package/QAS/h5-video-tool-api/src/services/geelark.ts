...
/** 将视频（URL、base64 data URL 或本地路径）上传到 GeeLark，返回 resourceUrl */
async function uploadVideoToGeelark(videoInput: string): Promise<string> {
  ...
  if (videoInput.startsWith('data:video/')) {
    ...
    const res = await axios.get(videoInput, { responseType: 'arraybuffer', timeout: 120000 });
    buffer = Buffer.from(res.data);
  } else {
    const fp = path.isAbsolute(videoInput) ? videoInput : path.join(process.cwd(), videoInput);
    if (!fs.existsSync(fp)) {
      throw new Error(`无效的视频输入：需为 data URL、HTTP URL 或本地路径。路径不存在: ${videoInput}`);
    }
