...
import { ALL_GOBS_FEATURES, ALL_MATRIX_FEATURES, DEFAULT_GOBS_FEATURES, DEFAULT_MATRIX_FEATURES, normalizeIdArray, userToGobsSession, } from '../gobs/gobsAuthTypes.js';
import { fetchMatrixGroupTagOptions, loadPublishAccountsFromConfig } from '../gobs/gobsPublishCatalog.js';
import { coerceGobsUserFeatures, countGobsSuperAdmins, deleteGobsUserById, findGobsUserByEmail, findGobsUserById, listGobsUsers, loadGobsUsersBlob, newGobsUserId, upsertGobsUser, } from '../gobs/gobsAuthStore.js';
import { hashGobsPassword, verifyGobsPassword } from '../gobs/gobsAuthPassword.js';
import { COOKIE_NAME, getGobsAuthCookieDeleteOptions, getGobsAuthCookieWriteOptions, signGobsSessionToken, signMatrixBridgeToken, verifyGobsSessionToken, } from '../gobs/gobsAuthSession.js';
const router = Router();
...
router.get('/matrix-tcp-probe', async (req, res) => {
    try {
        ...
        const allowLocal = rawHost === 'localhost' ||
            rawHost === '127.0.0.1' ||
            rawHost === '::1' ||
            /^(10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[0-1])\.)\d{1,3}\.\d{1,3}$/.test(rawHost);
        if (!allowLocal) {
            return res.json({ ok: true, skipped: true, host: rawHost, port });
        }
        ...
        res.json({ ok, skipped: false, host: rawHost, port });
    }
    catch (e) {
        res.status(500).json({ error: e instanceof Error ? e.message : 'failed' });
    }
...
router.post('/users', async (req, res) => {
    try {
        ...
        const { features, matrixFeatures } = coerceGobsUserFeatures(body.features ?? DEFAULT_GOBS_FEATURES, body.matrixFeatures ?? DEFAULT_MATRIX_FEATURES, isSuperAdmin);
        const now = Date.now();
...
router.patch('/users/:id', async (req, res) => {
    try {
        ...
        const { features, matrixFeatures } = coerceGobsUserFeatures(body.features ?? next.features, body.matrixFeatures ?? next.matrixFeatures, next.isSuperAdmin);
        ...
        if (next.isSuperAdmin) {
            delete next.publishAccountIds;
            delete next.matrixAllowedGroups;
        }
