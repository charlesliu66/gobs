...
function parseVeoError(stderr, modelHint) {
    if (!stderr)
        return null;
...
export async function generateVideoWithPython(options) {
    ...
    if (options.model)
        env.VEO_MODEL = options.model;
    if (options.duration != null)
        env.VEO_DURATION = String(options.duration);
    if (options.resolution)
        env.VEO_RESOLUTION = options.resolution;
    ...
    return new Promise((resolve, reject) => {
        ...
        proc.on('close', async (code) => {
            if (code !== 0) {
                const errMsg = parseVeoError(stderr, options.model ?? process.env.COMPASS_VIDEO_MODEL) ||
                    stderr ||
                    `Python 脚本退出码 ${code}`;
