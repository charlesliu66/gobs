...
**Previous update - v0.194**
- `/distribute` is lighter internally: asset-option helpers for package/current/local/output videos now live in a typed helper module instead of the page component.
- Distribution publish behavior, account selection, caption generation, recent context restore, and history remain unchanged.
- Helper tests now guard asset IDs, URL priority, dedupe ordering, prompt fallback, and source labels.

**Previous update - v0.193**
...
- **[context restore] Added active Distribution context persistence** (`h5-video-tool/src/components/distribute/distributionRecentContext.ts`, `h5-video-tool/src/pages/TabDistribute.tsx`): refresh can restore Package, selected asset, selected accounts, platform drafts, active draft, and publish options without auto-publishing.
- **[account groups] Added group preview/update helpers** (`h5-video-tool/src/utils/accountGroups.ts`, `h5-video-tool/src/components/distribute/AccountGroupPicker.tsx`): operators can inspect group members and update custom group membership from the current selection.
- **[failure guidance] Added batch next-action copy** (`h5-video-tool/src/components/distribute/DistributeStepPublish.tsx`, `h5-video-tool/src/components/distribute/distributePageViewModel.ts`): failures keep the raw reason visible and add a concrete next step for refresh/history/log evidence.
...
- **[quality panel] Added human-signal diagnostics** (`h5-video-tool/src/components/campaign/CreativeQualityPanel.tsx`, `h5-video-tool/src/components/campaign/CreativeFeedbackBar.tsx`): produced outputs now show current quality, feedback tags, static recommendations, and copy that states the panel uses human marks/tags/rules only.
- **[next version] Added child draft creation** (`h5-video-tool/src/components/campaign/feedback/creativeFeedbackActions.ts`, `h5-video-tool/src/pages/CampaignCreative.tsx`): Banner prompt and platform copy outputs can append a next-version draft with `parentOutputId`, inherited source asset IDs, Campaign/Brief context, feedback tags, issue tags, notes, and reviewer metadata.
- **[persistence] Added feedback metadata validation** (`h5-video-tool-api/src/services/campaignOutputPlan.ts`): output-plan persistence round-trips Run 4 metadata and rejects unsupported feedback tags without route rewrites.
...
- **[campaign production] Added deterministic text/post output production** (`h5-video-tool/src/components/campaign/outputPlan.ts`, `h5-video-tool/tests/campaignOutputProductionAdapter.test.ts`): supported `caption_set`, `headline_set`, `hashtag_set`, and `fb_post` items now become `produced` with visible draft content, while video/banner items stay blocked or ready without calling forbidden generation services.
- **[persistence] Extended output-plan payload validation** (`h5-video-tool-api/src/services/campaignOutputPlan.ts`, `h5-video-tool-api/tests/campaignOutputPlan.test.ts`): `producedOutputs` are validated, preserved, and rejected when malformed.
- **[distribution bridge] Fed produced text into pending packages** (`h5-video-tool/src/components/campaign/distributionPackage.ts`, `h5-video-tool/src/pages/CampaignCreative.tsx`): produced copy fills package caption/headline/hashtags, but package review, account selection, media readiness, and final publish remain explicit.
- **[tests/build] Added adapter/UI/API coverage and reran builds**: targeted frontend/backend tests pass, plus backend and frontend production builds.
...
- **[storage contract] Clarified Campaign Distribution package storage** (`docs/workflow/runs/2026-05-07-campaign-to-distribution-handoff-mvp/planner-spec.md`): use the existing `assetDb.ts` better-sqlite3 database with a `campaign_distribution_packages` table, indexed owner/status/time columns, and `payload_json` for the full package body.
- **[integration budget] Added a `TabDistribute` integration boundary**: if direct package intake requires broad publish/account rewiring, Builder should split to a smaller Pending Packages entry/panel instead of risking the existing distribution flow.
- **[missing asset action] Added `needs_asset` next-action guidance**: draft packages should lead operators toward asset-library selection, Quick Film generation, or Editor fine-tuning instead of a dead-end draft.
