...
| 部署 | 云主机 PM2（进程名 **`qas-api`**）、静态资源由 Nginx 提供 |
...
团队规范：**代码 → 提交 → 推送 → 本地构建 →（脚本）上传产物 → PM2 重启**。  
服务器上多为 **无 git**，部署靠上传**编译后**的 `api` 与 `frontend` 文件。

典型服务器布局（示例）：

```
/home/ubuntu/qas-h5/
├── api/           # 对应本仓库 h5-video-tool-api 的 dist/ 产出
├── frontend/      # 对应 h5-video-tool 的 dist/
└── scripts/
```

- PM2 进程：`qas-api`，入口如 `/home/ubuntu/qas-h5/api/index.js`。  
- 连接方式与密码等：见根目录 `CLAUDE.md` / `AGENTS.md`（不把密码写进本文档）。

**注意**：纯本地实验可跳过服务器；若用户明确要求「上生产」，必须完整流程且 **TypeScript 构建通过**。
