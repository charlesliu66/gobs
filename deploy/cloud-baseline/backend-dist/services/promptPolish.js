...
export async function polishPrompt(rawPrompt, options) {
    ...
    let systemPrompt;
    if (template) {
        const base = template.id === 'viral-dance'
...
${styleHint}`;
    ...
    try {
        ...
        const searchKeywords = Array.isArray(parsed.searchKeywords)
            ? parsed.searchKeywords.filter((k) => typeof k === 'string' && k.length > 0)
            : [];
        const folderHints = Array.isArray(parsed.folderHints)
            ? parsed.folderHints.filter((k) => typeof k === 'string' && /^(Scenario|Character|Weapon|Effect)$/i.test(k))
            : undefined;
        const result = {
        ...
        let characters = Array.isArray(parsed.characters)
            ? parsed.characters.filter((c) => typeof c === 'string' && c.trim().length > 0)
            : undefined;
        ...
        if ((template?.outputMode === 'multi-shot' || useCustomMultishot) && Array.isArray(parsed.shots) && parsed.shots.length > 0) {
            ...
            if (template?.id === 'boss-showcase') {
                ...
                while (normalized.length < 3 && parsed.shots[normalized.length]) {
                    ...
                    if (p)
                        normalized.push({ duration: 5, prompt: p });
                }
...
function buildFallbackFromPrompt(userText) {
    const t = (userText || '').trim();
    if (!t)
        return { caption: 'This hit different 🔥 Worth the watch.', hashtags: '#fyp #foryou #viral #shorts' };
    const stripTech = t
...
export async function generateCaptionForPost(prompt, platforms, options) {
    ...
    const uniquePlatforms = platforms?.length && platforms.length > 0
...
export async function expandShortDramaFromIdea(rawIdea) {
    ...
    try {
        ...
        const scriptContent = typeof parsed.scriptContent === 'string' ? parsed.scriptContent.replace(/\\n/g, '\n').trim() : '';
        ...
        if (e instanceof Error && e.message === '模型返回为空')
            throw e;
        throw new Error('剧本解析失败，请重试或缩短创意描述');
    }
}
export async function translateCaptionForPost(caption, hashtags, targetLanguage) {
    const cap = (caption ?? '').trim();
    const tag = (hashtags ?? '').trim();
    if (!cap && !tag) {
        return { caption: cap, hashtags: tag || '#fyp #viral' };
    }
    const langName = targetLanguage === 'EN'
        ? '英文 (English)'
        : targetLanguage === 'CN'
            ? '中文 (Chinese)'
            : targetLanguage === 'TH'
