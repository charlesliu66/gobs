...
- Release recommendation: GO for commit/push, staging validation, and metadata-only prod maintenance. After VPN connectivity was restored, staging smoke passed at `37ac488`: version endpoint, environment marker, frontend asset manifest, key routes, and `/api/prompt/templates` all matched the expected Phase 1 result. `release-ready.json` was written for staging with `commitShort=37ac488`. User approved a metadata-only prod maintenance action, `prod/api/build-info.json` was restored to deployed code commit `37ac488`, and prod quick smoke now passes for version endpoint, environment marker, frontend asset manifest, key routes, and `/api/prompt/templates`.
