...
function charFilePath(id) {
    return path.join(LIB_DIR, `${id}.json`);
}
// ── 保存/更新角色 ─────────────────────────────────────────────────────────────
characterLibraryRouter.post('/save', async (req, res) => {
    const body = req.body;
    if (!body.name?.trim()) {
    ...
    let createdAt = now;
    try {
        const existing = JSON.parse(await fs.readFile(charFilePath(id), 'utf-8'));
...
// ── 列出所有角色 ──────────────────────────────────────────────────────────────
characterLibraryRouter.get('/list', async (_req, res) => {
    await ensureDirs();
    ...
    for (const f of files) {
        try {
            const char = JSON.parse(await fs.readFile(path.join(LIB_DIR, f), 'utf-8'));
            list.push({
                id: char.id,
                name: char.name,
...
// ── 获取单个角色 ──────────────────────────────────────────────────────────────
characterLibraryRouter.get('/:id', async (req, res) => {
    const { id } = req.params;
    if (!/^[\w-]+$/.test(id)) {
...
// ── 删除角色 ─────────────────────────────────────────────────────────────────
characterLibraryRouter.delete('/:id', async (req, res) => {
    const { id } = req.params;
    if (!/^[\w-]+$/.test(id)) {
        res.status(400).json({ error: '无效 id' });
        return;
...
// ── 生成分享 token ────────────────────────────────────────────────────────────
characterLibraryRouter.post('/:id/share', async (req, res) => {
    const { id } = req.params;
    if (!/^[\w-]+$/.test(id)) {
        res.status(400).json({ error: '无效 id' });
        return;
...
// ── 通过分享 token 读取（只读） ────────────────────────────────────────────────
characterLibraryRouter.get('/share/:token', async (req, res) => {
    ...
    try {
        const payload = JSON.parse(await fs.readFile(path.join(SHARE_DIR, `${token}.json`), 'utf-8'));
...
// ── 通过分享 token 导入到本地形象库 ──────────────────────────────────────────
characterLibraryRouter.post('/import', async (req, res) => {
    ...
    try {
        ...
        const imported = { ...payload.char, id: newId, createdAt: now, updatedAt: now };
