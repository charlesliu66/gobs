...
import { sumCompassUsage } from './editorLlmUsage.js';
...
function buildUserPayload(input, extra) {
...
function snapClipToCandidateWindows(vc, windows) {
    if (windows.length === 0)
        return vc;
...
/** 校验并修正 LLM 输出的工程 */
export function sanitizeAgentProject(raw, allowedIds, durationByAssetId, candidateWindows) {
    const tracks = raw.tracks.map((t) => {
        ...
        for (const c of t.clips) {
            ...
            const shotIndex = typeof vc.shotIndex === 'number' && Number.isFinite(vc.shotIndex)
                ? Math.round(vc.shotIndex)
                : undefined;
            const note = typeof vc.note === 'string' ? vc.note.slice(0, 800) : undefined;
            const transitionAfter = vc.transitionAfter === 'crossfade' ? 'crossfade' : undefined;
            clips.push({
                ...next,
