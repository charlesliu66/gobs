...
export function TabMaterials() {
  ...
  const [backendCheckMsg, setBackendCheckMsg] = useState<string | null>(null);

  const handleCheckBackend = useCallback(async () => {
    setBackendCheckMsg('检查中…');
    const r = await checkBackendHealth();
    setBackendCheckMsg(r.ok ? '✓ 后端连接正常' : `✗ ${r.error}`);
  }, [checkBackendHealth]);
  ...
  return (
    <div className="min-h-screen p-6 lg:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        <h1 className="text-xl font-semibold text-[var(--color-text)]">素材管理</h1>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <button
                ...
                className="inline-flex items-center gap-2 px-3 py-2 border border-[var(--color-border)] text-[var(--color-text)] rounded-lg hover:bg-[var(--color-surface-hover)] text-sm"
              >
                检查后端连接
              </button>
              <button
              ...
              {backendCheckMsg && (
                <span className={`text-sm ${backendCheckMsg.startsWith('✓') ? 'text-[var(--color-success)]' : 'text-[var(--color-error)]'}`}>
                  {backendCheckMsg}
                </span>
              )}
            </div>
        ...
        {materialsCtx.verifiedFolderId && materialsCtx.verifiedFolderName && (
          <section>
            <h2 className="text-sm font-medium text-[var(--color-text)] mb-4">素材库内容</h2>
            <DriveExplorer
        ...
        {selectedOrder.length > 0 && (
          <section className="p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-elevated)]">
            <h2 className="text-sm font-medium text-[var(--color-text)] mb-4">当前已选素材</h2>
