...
function loadFolderSemantics(): Record<string, { folderPatterns: string[] }> {
  const candidates = [
 ...
 * 验证当前账号是否有该文件夹的访问权限
 */
driveRouter.post('/verify-folder', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (axios.isAxiosError(err)) {
      ...
      const googleMsg = (body as { error?: { message?: string } })?.error?.message;
    ...
    const data = axios.isAxiosError(err) ? err.response?.data as { error?: { message?: string } } | undefined : undefined;
...
async function collectFolderIds(
  ...
  while (depth < maxDepth && current.length > 0) {
    ...
    for (const folderId of current) {
      try {
        const { data } = await axios.get<{ files?: Array<{ id: string; mimeType: string }> }>(
...
async function collectFoldersWithNames(
  token: string,
  rootId: string,
  maxDepth = 3,
  ...
  const fetchFolderMeta = async (id: string): Promise<{ id: string; name: string } | null> => {
    try {
  ...
  if (rootMeta) result.push(rootMeta);
  ...
  while (depth < maxDepth && current.length > 0) {
    ...
    for (const folderId of current) {
      try {
        const { data } = await axios.get<{ files?: Array<{ id: string; name: string }> }>(
          'https://www.googleapis.com/drive/v3/files',
          {
            params: {
              q: `'${folderId.replace(/'/g, "\\'")}' in parents and mimeType='application/vnd.google-apps.folder'`,
...
function filterFolderIdsByHints(
  ...
  if (patternsToMatch.size === 0) return folders.map((f) => f.id);
...
driveRouter.post('/search', async (req: Request, res: Response) => {
  const authHeader = req.headers.authorization;
  if (!authHeader?.startsWith('Bearer ')) {
  ...
  const { keywords = [], folderId, folderHints } = req.body as {
    keywords?: string[];
    folderId?: string;
 ...
 */
driveRouter.post('/list', async (req: Request, res: Response) => {
  const authHeader = req.headers.authorization;
  if (!authHeader?.startsWith('Bearer ')) {
    res.status(401).json({ error: '请先连接 Google Drive' });
    return;
  ...
  try {
    const { data } = await axios.get<{ files: DriveItem[] }>(
      'https://www.googleapis.com/drive/v3/files',
      {
        params: {
 ...
 */
driveRouter.get('/thumbnail', async (req: Request, res: Response) => {
  const authHeader = req.headers.authorization;
  if (!authHeader?.startsWith('Bearer ')) {
    res.status(401).json({ error: '请先连接 Google Drive' });
    return;
  ...
  const mimeTypeParam = (req.query.mimeType as string | undefined) || '';
  ...
  const headers = { Authorization: `Bearer ${token}` };
  const opts = { responseType: 'arraybuffer' as const, timeout: 10000 };

  const sendBuffer = (buf: Buffer, contentType: string) => {
    res.setHeader('Content-Type', contentType);
    res.setHeader('Cache-Control', 'private, max-age=300');
    res.send(buf);
  };

  const tryAltMedia = async (contentTypeFallback: string): Promise<boolean> => {
    try {
      const { data, status, headers: resHeaders } = await axios.get<ArrayBuffer>(
      ...
      const ct = (resHeaders as Record<string, string>)['content-type']?.split(';')[0]?.trim() || contentTypeFallback;
  ...
  try {
    ...
    if (mimeTypeParam.startsWith('image/')) {
      if (await tryAltMedia(mimeTypeParam)) return;
    }

    // 2. 获取文件元信息（未传 mimeType 或 alt=media 失败时）
    let meta: { thumbnailLink?: string; mimeType?: string } | null = null;
    try {
      const { data } = await axios.get<{ thumbnailLink?: string; mimeType?: string }>(
        `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}`,
        {
    ...
    if (meta?.mimeType?.startsWith('image/')) {
      if (await tryAltMedia(meta.mimeType || 'image/png')) return;
    }

    // 4. thumbnailLink + Bearer
    if (meta?.thumbnailLink) {
      try {
        const { data, status } = await axios.get<ArrayBuffer>(meta.thumbnailLink, {
          ...opts,
          headers,
          validateStatus: (s) => s < 500,
        });
    ...
    try {
      const { data, status } = await axios.get<ArrayBuffer>(thumbUrl, {
        ...opts,
        validateStatus: (s) => s < 500,
      });
      if (status < 400) {
    ...
    if (await tryAltMedia('image/png')) return;
