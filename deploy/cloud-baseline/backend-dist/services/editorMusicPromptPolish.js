...
function sliceFirstBalancedJsonObject(s) {
    const i = s.indexOf('{');
    if (i < 0)
        return null;
    let depth = 0;
...
function tryLenientParse(jsonStr) {
    try {
...
function extractPromptFieldsLoose(text) {
    const p = /"prompt"\s*:\s*"((?:[^"\\]|\\.)*)"/s.exec(text) ??
        /'prompt'\s*:\s*'((?:[^'\\]|\\.)*)'/s.exec(text) ??
        /prompt\s*[:=]\s*"((?:[^"\\]|\\.)*)"/is.exec(text);
    const n = /"negativePrompt"\s*:\s*"((?:[^"\\]|\\.)*)"/s.exec(text) ??
        /'negativePrompt'\s*:\s*'((?:[^'\\]|\\.)*)'/s.exec(text) ??
        /negativePrompt\s*[:=]\s*"((?:[^"\\]|\\.)*)"/is.exec(text);
    const prompt = p?.[1]?.replace(/\\"/g, '"').replace(/\\n/g, '\n').trim() ?? '';
    if (!prompt)
        return null;
    const negativePrompt = n?.[1]?.replace(/\\"/g, '"').trim() ?? 'vocals, singing, speech, lyrics';
    return { prompt, negativePrompt };
}
export async function polishLyriaMusicPrompt(raw) {
    ...
    let parsed = tryLenientParse(jsonStr);
    if (parsed === undefined) {
    ...
    const negativePrompt = typeof p.negativePrompt === 'string' ? p.negativePrompt.trim() : 'vocals, singing, speech';
    if (!prompt)
        throw new Error('未生成有效 prompt');
    return { prompt, negativePrompt };
}
